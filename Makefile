.PHONY: all build build-linux install uninstall clean test lint help
...
install: build
	...
	# Install .env example if not exists
	if [ ! -f $(CONFIG_DIR)/.env ]; then \
		install -m 600 .env.example $(CONFIG_DIR)/.env; \
		echo "✓ Installed .env template to $(CONFIG_DIR)/.env"; \
		echo "⚠ Please edit $(CONFIG_DIR)/.env and set your TOKEN_DEVICE"; \
	else \
		echo "⚠ .env already exists at $(CONFIG_DIR)/.env (not overwritten)"; \
	fi
	...
	@echo ""
	@echo "Next steps:"
	@echo "  1. Set token: sudo nano $(CONFIG_DIR)/.env"
...
## run: Run the agent locally (for development)
run:
	@echo "Running $(BINARY_NAME) in development mode..."
	@if [ -z "$$TOKEN_DEVICE" ]; then \
		echo "Error: TOKEN_DEVICE environment variable not set"; \
		echo "Load from .env: export \$$(cat .env | xargs)"; \
		exit 1; \
	fi
	$(GOCMD) run ./cmd/agent start
