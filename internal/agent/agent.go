...
// updateCollectionInterval updates the collection interval dynamically
func (a *Agent) updateCollectionInterval(newInterval time.Duration) {
	a.mu.Lock()
	defer a.mu.Unlock()

	oldInterval := a.config.Collection.Interval
	a.config.Collection.Interval = newInterval
...
func (a *Agent) reloadConfig() error {
	...
	// Determine config path (use default if not available)
	configPath := "/etc/monify-agent/config.yaml"

	a.logger.Info("Reloading configuration from file")
	...
	// Preserve token from current config (don't reload from file)
	newConfig.Server.Token = a.config.Server.Token
